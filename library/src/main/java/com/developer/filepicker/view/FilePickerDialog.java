...
public class FilePickerDialog extends Dialog implements AdapterView.OnItemClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (size == 0 && properties.selection_type != DialogConfigs.CURRENT_DIR_SELECT) {
		...
		select.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View view) {
				if (properties.selection_type == DialogConfigs.CURRENT_DIR_SELECT) {
					String[] paths = new String[]{dir_path.getText().toString()};
					if (callbacks != null) {
		...
		mFileListAdapter.setNotifyItemCheckedListener(new com.developer.filepicker.controller.NotifyItemChecked() {
			public final void notifyCheckBoxIsClicked() {
				...
				if (size1 == 0 && properties.selection_type != DialogConfigs.CURRENT_DIR_SELECT) {
					select.setEnabled(false);
					int color;
	...
	protected void onStart() {
		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
			if (Utility.checkMediaAccessPermissions(context)) {
				...
				Log.d(TAG,
						"Permissions are not granted. You need to ask permission to user before accessing the storage and showing dialog.");
			}
		} else {
			if (Utility.checkStorageAccessPermissions(context)) {
				...
				Log.d(TAG,
						"Permission not granted. You need to ask permission to user before accessing the storage and showing dialog.");
			}
		}
	}
